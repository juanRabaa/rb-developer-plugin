...
class RB_Terms_List_Column extends RB_Objects_List_Column{
    ...
    protected function setup_screen_column($admin_page){
        RB_Filters_Manager::add_filter( "rb_tax_metabox-{$this->id}__{$admin_page}-column_base", self::manage_column_base_filter_tag($admin_page), array($this, 'add_column_base') );
        RB_Filters_Manager::add_filter( "rb_tax_metabox-{$this->id}__{$admin_page}-column_content", "manage_{$admin_page}_custom_column", array($this, 'add_column_content'), array(
            'accepted_args'  => 3,
        ));
    }
